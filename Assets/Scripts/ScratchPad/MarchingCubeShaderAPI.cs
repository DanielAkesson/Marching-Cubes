...
public class MarchingCubeShaderAPI
{
    ...
    public MarchingCubeShaderAPI(out float[] pointCloud, out Vector3[] verts, out int[] tris)
    {
        ...
        int kernel = marchShader.FindKernel("CSMain");
        marchShader.SetBuffer(kernel, "PointCloud", pointCloud_buf);
        marchShader.SetBuffer(kernel, "Verts", vert_buf);
        marchShader.SetBuffer(kernel, "Tris", tris_buf);
    }
    ...
    public void MarchCloudAsync(Action<AsyncGPUReadbackRequest> vertCallback, Action<AsyncGPUReadbackRequest> trisCallback, ref float[] pointCloud)
    {
        pointCloud_buf.SetData(pointCloud);
        marchShader.Dispatch(marchShader.FindKernel("CSMain"), 1, 1, 1);
        AsyncGPUReadback.Request(vert_buf, vertCallback);
        AsyncGPUReadback.Request(tris_buf, trisCallback);
    }
